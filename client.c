...
Graph* initialise(int size)
{
    ...
    g->n = size;//n=21
    g->graph = malloc(size * sizeof(Head));//head *graph=malloc(21*sizeof(node)) //21 nodes 
...
Graph* file(void)
{
    ...
    Graph* g = initialise(atoi(test) + 1);//initialize(21) // Adding 1 so can refer with vertex ids instead of converting to indices
